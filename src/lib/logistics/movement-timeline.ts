...
function canonicalMovementRows(): MovementTimelineRow[] {
...
export async function fetchNationalMovementTimelineSourced(limit = 80): Promise<SourcedResult<MovementTimelineRow[]>> {
  try {
    ...
    if (error || !data?.length) {
      return sourced(
        canonicalMovementRows().slice(0, limit),
        pilotSource("inventory_movements empty → MINISTRY_INVENTORY_MOVEMENTS"),
      );
    }
    ...
    return sourced(rows, liveSource("inventory_movements"));
  } catch {
    return sourced(
      canonicalMovementRows().slice(0, limit),
      pilotSource("inventory_movements unreachable → MINISTRY_INVENTORY_MOVEMENTS"),
    );
  }
}
