...
import {
  buildUnifiedVerificationQueue,
  type VerificationGridRow,
  type VerificationQueueDetail,
...
export default function VerificationQueueWorkspace() {
  ...
  const [rows, setRows] = React.useState<VerificationGridRow[]>(() => buildUnifiedVerificationQueue());
  ...
  const patchDetail = React.useCallback((id: string, fn: (d: VerificationQueueDetail) => VerificationQueueDetail) => {
    setRows((prev) =>
      prev.map((r) => {
        if (r.id !== id) return r;
  ...
  const runAction = React.useCallback(
    async (
      ...
      const actor = reviewerLabel || "Reviewer";
      ...
      patchDetail(id, (d) => ({
        ...d,
        status: nextStatus!,
        auditTimeline: [...d.auditTimeline, { at: iso, actor, stage: "workflow", note: auditNote }],
      }));
